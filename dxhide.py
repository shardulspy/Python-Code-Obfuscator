...
def dxhide(content: str, key: int) -> str:
    _content_ = Key.encrypt(content, key=key)
    _lines_sep_ = '/'
    content = _lines_sep_.join(hexlify(x.encode()).decode() for x in _content_)

    _names_ = ["_eval", "_exec", "_byte", "_bytes", "_bit", "_bits", "_system", "_encode", "_decode", "_delete", "_exit", "_rasputin", "_dxhide"]
    ...
    _final_content_ = fr"""class dxhide():
...
dxhide(_n1_=False,_n2_=False,_sparkle='''{content}''')""".strip().replace("_n1_",n_1.removeprefix("self.")).replace("_n2_",n_2.removeprefix("self.")).replace("_n3_",n_3.removeprefix("self.")).replace("_n4_",n_4.removeprefix("self.")).replace("_n5_",n_5).replace("_n6_",n_6).replace("_n7_",n_7).replace("_n8_",n_8).replace("_n9_",n_9.removeprefix("self.")).replace("_n10_",n_10.removeprefix("self.")).replace("_n12_",n_12.removeprefix("self."))
...
def main():
    ...
    _file = Write.Input("Drag your Python file here -> ", Colors.red_to_yellow, interval=0.005)
    ...
    content = dxhide(content=content, key=key)
    with open(file, 'w', encoding='utf-8') as f:
        f.write(content)

    print('\n')

    print(f"""\033[1;32mEncrypting with DXHIDE... 
\033[0;31mUsing key: {key}...
\033[35mPreparing encryption...
\033[34mFinalizing content...""")
    ...
    print("""\033[0mPYC file operations...
""")

    print("\033[1;31mEncryption complete! Exiting...")
    return exit()
